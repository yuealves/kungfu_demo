...
L2StockTickDataField trans_tick(const KyStdSnpType& ky_snp_data, int32_t host_time) {
    ...
    //std::copy(std::begin(ky_snp_data.sPrefix), std::end(ky_snp_data.sPrefix), std::begin(l2_data.sPrefix));
    l2_data.nSyl1 = 0;
    l2_data.nSyl2 = 0;
    l2_data.nSD2 = 0;
...
void WriteStockTickDataToCSV(const std::vector<StockTickData> &marketDataList, const std::string &filePath)
{
    ...
    if (file.tellp() == 0) 
    {
        file << "htscsecurityid,mddate,mdtime,securityidsource,preclosepx,totalvolumetrade,totalvaluetrade,lastpx,openpx,highpx,lowpx,diffpx1,totalbuyqty,totalsellqty,weightedavgbuypx,weightedavgsellpx,"
            << "buypricequeue0,buypricequeue1,buypricequeue2,buypricequeue3,buypricequeue4,buypricequeue5,buypricequeue6,buypricequeue7,buypricequeue8,buypricequeue9,"
            << "buyorderqtyqueue0,buyorderqtyqueue1,buyorderqtyqueue2,buyorderqtyqueue3,buyorderqtyqueue4,buyorderqtyqueue5,buyorderqtyqueue6,buyorderqtyqueue7,buyorderqtyqueue8,buyorderqtyqueue9,"
            << "sellpricequeue0,sellpricequeue1,sellpricequeue2,sellpricequeue3,sellpricequeue4,sellpricequeue5,sellpricequeue6,sellpricequeue7,sellpricequeue8,sellpricequeue9,"
            << "sellorderqtyqueue0,sellorderqtyqueue1,sellorderqtyqueue2,sellorderqtyqueue3,sellorderqtyqueue4,sellorderqtyqueue5,sellorderqtyqueue6,sellorderqtyqueue7,sellorderqtyqueue8,sellorderqtyqueue9,"
            << "buyorderqueue0,buyorderqueue1,buyorderqueue2,buyorderqueue3,buyorderqueue4,buyorderqueue5,buyorderqueue6,buyorderqueue7,buyorderqueue8,buyorderqueue9,"
            << "sellorderqueue0,sellorderqueue1,sellorderqueue2,sellorderqueue3,sellorderqueue4,sellorderqueue5,sellorderqueue6,sellorderqueue7,sellorderqueue8,sellorderqueue9,"
            << "tradingphasecode,numtrades,afterprice,afterturnover,aftervolume,afternumtrades,receivetime\n";
    }
...
void WriteIndexTickDataToCSV(const std::vector<IndexTickData>& indexDataList, const std::string& filePath) {
    ...
    // 写入CSV头
    file << "mddate,mdtime,htscsecurityid,lastpx,highpx,lowpx,totalvolumetrade,totalvaluetrade,tradingphasecode,receivetime\n";

    // 写入数据
    for (const auto& data : indexDataList) {
        file << data.to_string() << "\n";
    }
...
void WriteFutureTickDataToCSV(const std::vector<FutureTickData>& futureDataList, const std::string& filePath) {
    ...
    std::ofstream file(filePath, std::ios::app); // 使用std::ios::app以追加模式打开文件
    if (!file.is_open()) {
        std::cerr << "Failed to open file: " << filePath << std::endl;
        return;
    }
    ...
    // 如果文件是新创建的，写入CSV头
    if (file.tellp() == 0) {
        file << "htscsecurityid,mddate,mdtime,securityidsource,preclosepx,totalvolumetrade,totalvaluetrade,lastpx,openpx,highpx,lowpx,preopeninterest,presettleprice,openinterest,"
            << "buypricequeue0,buypricequeue1,buypricequeue2,buypricequeue3,buypricequeue4,"
            << "buyorderqtyqueue0,buyorderqtyqueue1,buyorderqtyqueue2,buyorderqtyqueue3,buyorderqtyqueue4,"
            << "sellpricequeue0,sellpricequeue1,sellpricequeue2,sellpricequeue3,sellpricequeue4,"
            << "sellorderqtyqueue0,sellorderqtyqueue1,sellorderqtyqueue2,sellorderqtyqueue3,sellorderqtyqueue4,"
            << "tradingphasecode,receiveTime\n";
    }

    // 写入数据
    for (const auto& data : futureDataList) {
        file << data.to_string() << "\n";
    }
...
void WriteKlineDataToCSV(const std::vector<KlineData>& klineDataList, const std::string& filePath) {
    ...
    // 写入CSV头
    if (file.tellp() == 0) {
        file << "htscsecurityid,mddate,mdtime,openpx,closepx,highpx,lowpx,numtrades,totalvolumetrade,totalvaluetrade,receivetime\n";
    }
    // 写入数据
    for (const auto& data : klineDataList) {
        file << data.to_string() << "\n";
    }
...
void WriteStockOrderDataToCSV(const std::vector<StockOrderData> &stock_order_list, const std::string& filePath){
    ...
    // 写入CSV头
    if (file.tellp() == 0) {
        file << "htscsecurityid,mddate,mdtime,securityidsource,securitytype,orderindex,ordertype,orderprice,orderqty,orderbsflag,orderno,applseqnum,channelno,receivetime\n";
    }
    // 写入数据
    for (const auto& data : stock_order_list) {
        file << data.to_string() << "\n";
    }
...
void WriteStockTransactionDataToCSV(const std::vector<StockTransaction>& transaction_list,const std::string& filePath){
    ...
    // 写入数据
    for (const auto& data : transaction_list) {
        file << data.to_string() << "\n";
    }
