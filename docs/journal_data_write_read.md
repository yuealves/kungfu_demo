...
DataConsumer::run()                         ← 本模块入口  § 2
        │
        ├─→ getJournalFiles()               § 2.1  查找 journal 文件 (src/data_consumer.cpp:107-125)
        │       └─→ 遍历目录，匹配 yjj.<channel>.*.journal
        │
        ├─→ LocalJournalPage::load()        § 2.2  mmap 加载页面 (src/data_consumer.cpp:62-84)
        │       └─→ open() + mmap() 系统调用
        │
        └─→ 循环读取帧
                │
                ├─→ nextFrame()             § 2.3  读取下一帧 (src/data_consumer.cpp:86-99)
                │       └─→ 解析 LocalFrameHeader
                │
                ├─→ getFrameData()          § 2.4  获取帧数据 (src/data_consumer.cpp:101-103)
                │       └─→ 跳过 header，取 payload
                │
                └─→ 消息分发（按 msg_type）
                        │
                        ├─→ MSG_TYPE_L2_TICK (61)
                        │       └─→ on_market_data()     § 2.5 (src/data_consumer.h:21-31)
                        │               └─→ trans_tick()  § 3.1 (src/insight_types.cpp:3-74)
                        │
                        ├─→ MSG_TYPE_L2_ORDER (62)
                        │       └─→ on_order_data()      § 2.6 (src/data_consumer.h:32-46)
                        │               └─→ trans_order() § 3.2 (src/insight_types.cpp:76-114)
                        │
                        └─→ MSG_TYPE_L2_TRADE (63)
                                └─→ on_trade_data()      § 2.7 (src/data_consumer.h:48-62)
                                        └─→ trans_trade() § 3.3 (src/insight_types.cpp:116-161)
```
