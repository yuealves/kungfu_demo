...
当前 `realtime_kbar` 不会处理所有 journal 中出现的 symbol，而是会先做一层 universe 过滤。

当前保留规则：

- `0 <= symbol < 700000`
- 且排除 `200000 <= symbol < 202000`

也就是：

- 保留当前项目需要的 A 股主 universe
- 排除 `200xxx`、`201xxx` 这批非目标 symbol
- `900xxx` 本来就不会进入当前实现，因为它们不满足 `< 700000`

这样做的原因不是单纯按“证券名称分类”过滤，而是按本项目的目标 universe 过滤：

- 这批 `200/201` 代码不在当前对齐使用的 JQ `price_1m` 股票 universe 里
- 它们的时延分布明显更不稳定
- 会显著增加 patch 数量和运行期噪声

在本地 replay 中，过滤 `200/201` 之后：

- `symbols_seen` 从 `5229` 降到 `5191`
- `patch_files` 从 `66` 降到 `0`

因此，对当前项目来说，排除这批非目标 symbol 是合理且必要的。
...
### 7.4 为什么一个分钟会产生多个 patch

当前实现允许同一个分钟生成多个 patch 文件，这是有意设计，不是异常行为。

原因是：

- 主文件发布后，该分钟并不会被“封死”
- 只要后续又有新的迟到 tick 影响这个分钟
- 该分钟就会再次进入 `dirty` 状态
- 每一波新的 `dirty` 更新，都会在 1 秒节流窗口到期后再输出一个新的 patch

可以把它理解成：

1. 分钟 `M` 的主文件先发布，记为 `version=1`
2. 第一波迟到更新到来，股票被加入 `dirty_symbols`
3. 满 1 秒后，输出 `kbar_patch_M_v2.csv`
4. 如果之后该分钟又有新的迟到更新，再次标记 `dirty_symbols`
5. 再过 1 秒，输出 `kbar_patch_M_v3.csv`
6. 如此重复

因此，一个分钟最终会产生几个 patch，取决于：

- 主文件发布后有几波迟到更新
- 这些迟到更新是否被 1 秒节流窗口合并到同一个 patch
- patch 刚写完后，是否又有新的 symbol 或新的 bar 更新继续到达

所以“多个 patch”本质上表示：

- 这个分钟在主文件发布后，发生了多次独立的后续修正波次
