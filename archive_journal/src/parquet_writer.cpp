...
// 纯 size-based 分片（不再强制按线程数拆分，单文件内部靠 Arrow 并行写列）
static size_t compute_rows_per_file(size_t total, size_t max_rows_per_file) {
    return (max_rows_per_file > 0 && total > max_rows_per_file) ? max_rows_per_file : total;
}
...
void write_tick_parquet(const std::vector<TickRecord>& records, const std::string& filepath,
                        size_t max_rows_per_file, int num_threads) {
    auto schema = tick_schema();
    size_t total = records.size();
    size_t rows_per_file = compute_rows_per_file(total, max_rows_per_file);
    ...
    auto worker = [&]() {
        while (true) {
            size_t file_idx = next_chunk.fetch_add(1);
    ...
    int actual_threads = std::min((size_t)num_threads, num_files);
    if (actual_threads <= 1) {
        worker();
    } else {
